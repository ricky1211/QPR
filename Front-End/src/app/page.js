...
export default function Home() {
  ...
  const handleApproveNcrAction = (id, ncrNum) => {
    setPendingNcrs(prev => prev.filter(n => n.id !== id));
    
    const newNotif = {
      id: Date.now(),
      message: `NCR ${ncrNum} berhasil disetujui digital.`,
      time: "Baru saja",
      type: "success",
      unread: true
    };
    setNotifications(prev => [newNotif, ...prev]);
    alert(`Sukses: NCR ${ncrNum} disetujui digital!`);
  };
  ...
  const handleApproveQprAction = (id, qprNum) => {
    ...
    const newNotif = {
      id: Date.now(),
      message: `Klaim QPR ${qprNum} telah disetujui digital dan diteruskan ke Keuangan (Finance).`,
      time: "Baru saja",
      type: "success",
      unread: true
    };
    setNotifications(prev => [newNotif, ...prev]);
    alert(`Sukses: Otoritas claim QPR ${qprNum} telah ditandatangani!`);
  };
