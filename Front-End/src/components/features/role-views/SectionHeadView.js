...
export default function SectionHeadView({ pendingNcrs, handleApproveNcrAction }) {
  const sectionHeadNcrs = pendingNcrs.filter((n) => n.requiredRole === "Section Head");
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-100 rounded-lg shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 bg-slate-50/30">
          ...
          <h4 className="text-sm font-bold text-slate-800 mt-1">NCR Menunggu Persetujuan Level 1</h4>
        </div>
                ...
                <div key={ncr.id} className="p-4 bg-slate-50 border border-slate-100 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <button
                      ...
                      className="flex-1 sm:flex-none px-3.5 py-2 border border-slate-200 hover:bg-slate-100 text-slate-600 rounded-md text-xs font-bold transition-colors"
                    >
                      Detail
                    </button>
                    <button
                      onClick={() => handleApproveNcrAction(ncr.id, ncr.ncrNumber)}
                      className="flex-1 sm:flex-none px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-md text-xs font-bold shadow-sm shadow-amber-500/10 transition-colors"
                    >
                      Setujui Level 1
                    </button>
                  </div>
