...
export default function ApproveNcrDashboard({ pendingNcrs, handleApproveNcrAction }) {
  const [levelTab, setLevelTab] = useState("level-1"); // 'level-1' or 'level-2'
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-5 bg-white border border-slate-100 rounded-lg shadow-sm gap-4">
        ...
        <div className="flex bg-slate-100 p-1 rounded-md shrink-0">
          <button
            onClick={() => setLevelTab("level-1")}
            className={`px-4 py-1.5 text-[11px] font-bold rounded-lg transition-all ${
              levelTab === "level-1" ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-900"
            }`}
          >
            Level 1: Section Head
          </button>
          <button
            onClick={() => setLevelTab("level-2")}
            className={`px-4 py-1.5 text-[11px] font-bold rounded-lg transition-all ${
              levelTab === "level-2" ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-900"
            }`}
          >
            Level 2: Dept Head
          </button>
